	...
	def upload_image():
		if request.method == "POST":
			...
			if img_file and img_file.filename[-3:] in allowed_extensions:
				...
				#return logits before softmax, try without attack
				#str(res)
				return "all_predict" + str(test) + "target class is " + str(target_class)
...
def to_matrix(img_file) -> np.ndarray:
	return imageio.imread(img_file, as_gray=True)
